       ...
       VA_LCS* const lcs)
{
    ...
    for (size_t i = context.diagonals[offset + idx]; i < end; ++i)
    {
        ...
        else if (matching)
        {
            ...
            va_array_append(context.allocator, lcs->nodes, ((VA_LCS_Node3) {match_row, match_col, length, false, 0, -1, -1}));
            ...
            {
                lcs->index[lcs_pos].head = va_array_length(lcs->nodes) - 1;
            } // else
            lcs->index[lcs_pos].tail = va_array_length(lcs->nodes) - 1;
         ...
         size_t const len_obs, char const observed[static restrict len_obs])
{
    ...
    lcs.index = allocator.alloc(allocator.context, NULL, 0, umin(len_ref, len_obs) * sizeof(*lcs.index));
    if (lcs.index == NULL)
    {
    ...
    lcs.index = allocator.alloc(allocator.context, lcs.index, umin(len_ref, len_obs) * sizeof(*lcs.index), lcs.length * sizeof(*lcs.index));
