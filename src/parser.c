    ...
    GVA_Variant variant[static restrict 1])
{
    ...
    if ((tok = match(len - idx, expression + idx, 3, "del")))
    {
        ...
        if ((tok = match(len - idx, expression + idx, 3, "ins")))
        {
            ...
            {
                idx += tok;
                variant->sequence = (GVA_String) {array_length(allele->inserted) - start, allele->inserted + start};
            } // if
        } // if
        return idx;
    } // if
    ...
    if ((tok = match(len - idx, expression + idx, 3, "dup")))
    {
        ...
        variant->sequence = (GVA_String) {length, allele->inserted + start};
        return idx;
    } // if
    ...
    if (idx < len && expression[idx] == '[')
    {
        ...
        if (idx < len && expression[idx] == ']')
        {
            ...
            if (variant->end - variant->start == 0)
            {
                ...
                variant->sequence = (GVA_String) {sequence.len * count, allele->inserted + start};
                return idx;
            } // if
